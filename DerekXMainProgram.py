...
def drawScore(window,score):
    ...
    screen.blit(scoreRender,(650-scoreRender.get_width(),55))
...
menuFont = font.SysFont("broadway",40)
scoreFont = font.SysFont("calibri",40)
...
while running:
    ...
    if mode == MENU:
        ...
        if not player.alive:
            ...
            rawDat = docR.readline()
            highscore = int(rawDat)
            print(highscore,'/',player.score)
            docR.close()
            if player.score > highscore:
                docW = open("DerekXHighscore.dat",'w')
                docW.write(str(highscore))
                docW.close()
